...
public class Net10PreCheck : PreCheckBase
{
    public Net10PreCheck()
        : base(".Net 10 Desktop Runtime", true) { }
    ...
    public override async Task<PreCheckResult> CheckOperation()
    {
        ...
        const string failedButtonText = "Download .Net 10 Desktop Runtime";
        ...
        try
        {
            ...
            if (!result.Succeeded)
            {
                return PreCheckResult.FromError(
                    result.Message + "\n\nYou most likely don't have .net 10 installed",
                    failedButtonText,
                    failedButtonAction
                );
            }
